...
所有 `TEST_SHAPES` 和 `GENERAL_SHAPES` 均执行 FP16/BF16 x int32/int64 的完整
笛卡尔积。`rank` 已包含在 shape 字段中，保证四种编译期 Generic rank 都覆盖四种
dtype/index 组合。
...
TEST_SHAPES = [
    ...
    ("Generic", "rank8 output tile", (47, 8, 4096, 1024, 1024, 0, "alternating", 1.0, "generic")),
    ...
    ("Generic", "unsorted nonzero slice", (64, 32, 257, 513, 577, 32, "mixed", 0.25, "generic")),
...
GENERAL_SHAPES = [
    ...
    ("OBoundary", "rank8 P-1", (3, 8, 64, 1023, 1023, 0, "mixed", 1.0, "generic")),
    ("OBoundary", "rank8 P", (3, 8, 64, 1024, 1024, 0, "single", 1.0, "generic")),
    ("OBoundary", "rank8 P+1", (3, 8, 64, 1025, 1041, 8, "all_negative", -0.5, "generic")),
    ...
    ("OBoundary", "rank32 P-1", (3, 32, 64, 255, 255, 0, "mixed", 1.0, "generic")),
    ("OBoundary", "rank32 P", (3, 32, 64, 256, 256, 0, "single", 1.0, "generic")),
    ("OBoundary", "rank32 P+1", (3, 32, 64, 257, 273, 8, "all_negative", -0.5, "generic")),
    ("OBoundary", "rank32 max O", (1, 32, 64, 16384, 16384, 0, "single", 1.0, "generic")),
    ("OBoundary", "rank64 P-1", (3, 64, 64, 127, 127, 0, "mixed", 1.0, "generic")),
    ("OBoundary", "rank64 P", (3, 64, 64, 128, 128, 0, "single", 1.0, "generic")),
    ("OBoundary", "rank64 P+1", (3, 64, 64, 129, 145, 8, "all_negative", 0.0, "generic")),
...
1. Fast 契约为 `rank=16 && H/O<=4096`；其余受支持组合进入 Generic group1。
   `M<=1024` 的 fast 用例必须额外核对 quotient/remainder 的 rowBegin/rowCount，
   `M>1024` 核对 group 对齐范围。
...
6. Generic 尾块矩阵通过后单独运行 mssanitizer；fast 与 Generic 各覆盖一次 ACL
   Graph capture/replay。真实权重性能结果另行记录，不能由精度矩阵推导。
