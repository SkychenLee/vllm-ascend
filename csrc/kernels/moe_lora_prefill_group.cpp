...
constexpr int32_t INVALID_PERM_RECORD = static_cast<int32_t>(0x80000000U);
...
public:
        ...
        uint32_t numCores, uint32_t blockDim, uint32_t prefixTileGroups)
    {
        ...
        // ProcessTile pads every core's row to eight int32 values.  Allocate
        // the physical padded pitch as well, including the G<8 case.
        const uint32_t prefixPitchGroups = (prefixTileGroups_ + 7U) & ~7U;
        const uint32_t packedBytes =
            numCores * prefixPitchGroups * static_cast<uint32_t>(sizeof(int32_t));
        pipe_->InitBuffer(countQueue_, 1, packedBytes);
        pipe_->InitBuffer(prefixQueue_, 1, packedBytes);
        pipe_->InitBuffer(totalQueue_, 1,
                          prefixPitchGroups * static_cast<uint32_t>(sizeof(int32_t)));
    }
...
public:
    ...
    __aicore__ inline void Init(
        ...
        __gm__ void* groupTotal, __gm__ void* groupedX,
        __gm__ void* permRecord,
    ...
    __aicore__ inline void Process()
    {
        ...
        if (blockIdx == 0) {
            MaterializeSinkRows(groupTotal, groupStart);
        }
...
private:
    __aicore__ inline void MaterializeSinkRows(
        const AscendC::LocalTensor<int32_t>& groupTotal,
        const AscendC::LocalTensor<int32_t>& groupStart)
    {
        int32_t rawGroup0End = groupTotal.GetValue(0);
        if (rawGroup0End < 0) {
            rawGroup0End = 0;
        }
        int32_t normalizedGroup0End = numGroups_ > 1U
            ? groupStart.GetValue(1)
            : static_cast<int32_t>(numRows_);
        if (normalizedGroup0End < 0) {
            normalizedGroup0End = 0;
        } else if (normalizedGroup0End > static_cast<int32_t>(numRows_)) {
            normalizedGroup0End = static_cast<int32_t>(numRows_);
        }
        if (rawGroup0End > normalizedGroup0End) {
            rawGroup0End = normalizedGroup0End;
        }
        for (int32_t row = rawGroup0End; row < normalizedGroup0End; ++row) {
            ZeroRow(static_cast<uint32_t>(row));
            WritePermRecord(static_cast<uint32_t>(row), INVALID_PERM_RECORD);
        }
    }
...
#define MOE_LORA_PREFILL_SCATTER_A2A_DECLARE(                                                 \
    ...
    extern "C" __global__ __aicore__ void                                                    \
        moe_lora_prefill_scatter_a2a_##DATA_NAME##_##COUNT_NAME##_##ENABLED_NAME(             \
            ...
            __gm__ void* groupTotal, __gm__ void* groupedX, __gm__ void* permRecord,          \
            uint32_t numRows,                                                                  \
    ...
    {                                                                                         \
        ...
        op.Init(x, expertCount, exchangedLoraIndices, adapterEnabled, corePrefix, groupStart,  \
                groupTotal, groupedX, permRecord, numRows, numAdapters, numExperts, numGroups, \
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t tileBegin = begin; tileBegin < end;
            ...
            for (uint32_t local = 0; local < tileRows; ++local) {
                ...
                if (encoded == INVALID_PERM_RECORD) {
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t tileBegin = begin; tileBegin < end;
            ...
            for (uint32_t local = 0; local < tileRows; ++local) {
                const int32_t encoded =
                    perm.GetValue(local * ERROR_RECORD_LANES);
                if (encoded >= 0 && static_cast<uint32_t>(encoded) < numRows_) {
...
extern void moe_lora_prefill_scatter_alltoall_impl(
    ...
    void* adapterEnabled, void* corePrefix, void* groupStart, void* groupTotal,
