...
#define BGMV_SHRINK_TYPE_DECLARE(TYPE, INDEX_TYPE, INDEX_SUFFIX)                                                       \
    extern "C" __global__ __aicore__ void bgmv_shrink_##TYPE##_##INDEX_SUFFIX(                                      \
    ...
    {                                                                                                                  \
        AscendC::TPipe pipe;                                                                                           \
        BGMVShrink<TYPE, INDEX_TYPE> op(&pipe);                                                                        \
...
BGMV_SHRINK_TYPE_DECLARE(half, int64_t, int64)
BGMV_SHRINK_TYPE_DECLARE(half, int32_t, int32)
#if !defined(__CCE_AICORE__) || (__CCE_AICORE__ >= 220)
    BGMV_SHRINK_TYPE_DECLARE(bfloat16_t, int64_t, int64)
    BGMV_SHRINK_TYPE_DECLARE(bfloat16_t, int32_t, int32)
#endif
...
extern void bgmv_shrink_impl(AscendType type, void* stream, void* x, void* weight, void* indices, uint32_t indicesSize,
                             ...
                             uint32_t maxLoRARank, float scale, bool indicesIsInt32)
{
    uint32_t blockDim = (batchSize + numTokensPerCore - 1) / numTokensPerCore;
    if (type == AscendType::FP16) {
        if (indicesIsInt32) {
            bgmv_shrink_half_int32<<<blockDim, nullptr, stream>>>(x, weight, indices, indicesSize, y, batchSize,
                                                                  numTokensPerCore, inputHiddenDim, maxLoRARank, scale);
        } else {
            bgmv_shrink_half_int64<<<blockDim, nullptr, stream>>>(x, weight, indices, indicesSize, y, batchSize,
                                                                  numTokensPerCore, inputHiddenDim, maxLoRARank, scale);
        }
    } else if (type == AscendType::BF16) {
        #if !defined(__CCE_AICORE__) || (__CCE_AICORE__ >= 220)
        if (indicesIsInt32) {
            bgmv_shrink_bfloat16_t_int32<<<blockDim, nullptr, stream>>>(
                x, weight, indices, indicesSize, y, batchSize, numTokensPerCore, inputHiddenDim, maxLoRARank, scale);
        } else {
            bgmv_shrink_bfloat16_t_int64<<<blockDim, nullptr, stream>>>(
                x, weight, indices, indicesSize, y, batchSize, numTokensPerCore, inputHiddenDim, maxLoRARank, scale);
        }
        #endif
    } else {
