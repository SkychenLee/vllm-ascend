...
| `enable_moe_lora_dual_stream`       | bool | `False` | Whether to overlap W8A8 MoE base expert computation and LoRA computation on separate NPU streams for pure decode batches in model runner V1. Prefill and mixed batches remain on one stream. This option applies to expert-parallel AllGather and AlltoAll execution and does not support fully sharded LoRA. |
